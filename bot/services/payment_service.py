...
from bot.locales.ru import PAYMENT_CHOICE, PAYMENT_SUCCESS
...
async def handle_payment_flow(bot, message, user_id, state, photo_path, gender):
    """
    ...
    if TEST_MODE:
        ...
        await process_successful_payment(bot, user_id, message.chat.id, "test", "0", photo_path, gender)
        return
    ...
    if is_free_user(user_id, username):
        ...
        await process_successful_payment(bot, user_id, message.chat.id, "whitelist", "0", photo_path, gender)
        return

    await state.update_data(photo_path=photo_path, gender=gender)
...
async def process_successful_payment(bot: Bot, user_id: int, chat_id: int, method: str, amount: str,
    ...
    msg = await bot.send_message(chat_id, PAYMENT_SUCCESS)
