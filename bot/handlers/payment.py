...
async def handle_payment_choice(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if method == "cancel":
        ...
        await bot.send_message(chat_id, ru.PHOTO_PROMPT)
        return
    ...
    if method == "stars":
        await send_stars_invoice(bot, chat_id, PRICE_STARS)
    elif method == "card":
        await send_card_invoice(bot, chat_id, CARD_PRICE_RUB)
    elif method == "crypto":
        try:
            ...
            await bot.send_message(chat_id, ru.CRYPTO_INVOICE_SENT, reply_markup=kb, parse_mode="HTML")
            
            # Start background polling
            asyncio.create_task(start_invoice_polling(bot, invoice_id, user_id, chat_id, photo_path, gender))
            
            # Transition state? Let's leave them in waiting_for_payment but they can't send new photos easily
            # until they cancel or it expires. Or we can set them to processing.
            await state.set_state(AnalysisFlow.processing)
            
        except Exception as e:
            logger.error(f"Crypto error: {e}")
            await bot.send_message(chat_id, ru.ERROR_GENERAL)
...
async def successful_payment(message: Message, state: FSMContext, bot: Bot):
    ...
    # Determine method based on currency
    ...
    if currency == "XTR":
        ...
        method = "card"
        amount = str(payment_info.total_amount / 100) # Assuming RUB and amount is in kopecks
